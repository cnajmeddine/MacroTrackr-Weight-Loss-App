...
<html lang="en">
  <head>
    <title>Produits céréaliers - macrotrackr</title>
  ...
  <body>
    <div>
      <link href="./cereals.css" rel="stylesheet" />

      <div class="cereals-container">
        <header data-role="Header" class="cereals-header">
          <a href="main-dashboard.php"><img
            alt="logo"
            src="public/playground_assets/logo-1400w.png"
            class="cereals-image"
          /></a>
          <div class="cereals-nav">
            <nav
              class="navigation-links-out-nav navigation-links-out-root-class-name30"
            >
        ...
        <h1 class="cereals-text">Calories dans les aliments</h1>
        <span>Entrez un nom pour rechercher un aliment spécifique:</span>
        <form action="" method="GET">
        
        <input type="text" name="search" required value="<?php if(isset($_GET['search'])){echo $_GET['search']; } ?>" class="cereals-searchbar input" placeholder="Chercher un aliment" />
        
        </form>
        <span class="cereals-text02">
          (Vous ne trouvez pas ce que vous cherchez ? - Entrez votre propre
          repas personnalisé <a href="add-meal-per.php">ici</a>)
        </span>
        
        <table class="cereals-list list">
          ...
          <?php
            if(isset($_GET['search']))
            {
                ...
                $query = "SELECT * FROM cata WHERE categorie='cereals' and aliment LIKE '%$filtervalues%' ";
          ...
          $sql= "SELECT * FROM cata WHERE categorie='cereals'; ";
