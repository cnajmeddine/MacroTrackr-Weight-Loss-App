...
<html lang="en">
  <head>
    <title>Viande, poisson et fruits de mer - macrotrackr</title>
  ...
  <body>
    <div>
      <link href="./meats.css" rel="stylesheet" />

      <div class="meats-container">
        <header data-role="Header" class="meats-header">
          ...
          <div class="meats-nav">
            <nav
              class="navigation-links-out-nav navigation-links-out-root-class-name11"
            >
        ...
        <h1 class="meats-text">Calories dans les aliments</h1>
        <span>Entrez un nom pour rechercher un aliment spécifique:</span>
        <form action="" method="GET">

        <input type="text" name="search" required value="<?php if(isset($_GET['search'])){echo $_GET['search']; } ?>" class="meats-searchbar input" placeholder="Chercher un aliment" />

        </form>
        <span class="meats-text02">
          (Vous ne trouvez pas ce que vous cherchez ? - Entrez votre propre
          repas personnalisé <a href="add-meal-per.php">ici</a>)
        </span>


        <table class="meats-list list">
          ...
          if(isset($_GET['search']))
          {
              ...
              $query = "SELECT * FROM cata WHERE categorie='viandes' and aliment LIKE '%$filtervalues%' ";
          ...
          $sql= "SELECT * FROM cata WHERE categorie='viandes'; ";
