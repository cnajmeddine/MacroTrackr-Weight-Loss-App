...
<html lang="en">
  <head>
    <title>Produits laitiers - macrotrackr</title>  
  ...
  <body>
    <div>
      <link href="./dairy.css" rel="stylesheet" />

      <div class="dairy-container">
        <header data-role="Header" class="dairy-header">
          ...
          <div class="dairy-nav">
            <nav
              class="navigation-links-out-nav navigation-links-out-root-class-name29"
            >
        ...
        <h1 class="dairy-text">Calories dans les aliments</h1>
        <span>Entrez un nom pour rechercher un aliment spécifique:</span>
        <form action="" method="GET">

        <input type="text" name="search" required value="<?php if(isset($_GET['search'])){echo $_GET['search']; } ?>" class="dairy-searchbar input" placeholder="Chercher un aliment" />

        </form>
        <span class="dairy-text02">
          (Vous ne trouvez pas ce que vous cherchez ? - Entrez votre propre
          repas personnalisé <a href="add-meal-per.php">ici</a>)
        </span>
        
        <table class="dairy-list list">
          ...
          <?php
            if(isset($_GET['search']))
            {
                ...
                $query = "SELECT * FROM cata WHERE categorie='laitiers' and aliment LIKE '%$filtervalues%' ";
          ...
          $sql= "SELECT * FROM cata WHERE categorie='laitiers'; ";
