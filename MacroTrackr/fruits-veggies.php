...
<html lang="en">
  <head>
    <title>Fruits et légumes - macrotrackr</title>
  ...
  <body>
    <div>
      <link href="./fruits-veggies.css" rel="stylesheet" />

      <div class="fruits-veggies-container">
        <header data-role="Header" class="fruits-veggies-header">
          <a href="main-dashboard.php"><img
            alt="logo"
            src="public/playground_assets/logo-1400w.png"
            class="fruits-veggies-image"
          /></a>
          <div class="fruits-veggies-nav">
            <nav
              class="navigation-links-out-nav navigation-links-out-root-class-name23"
            >
        ...
        <h1 class="fruits-veggies-text">Calories dans les aliments</h1>
        <span>Entrez un nom pour rechercher un aliment spécifique:</span>
        <form action="" method="GET">

        <input type="text" name="search" required value="<?php if(isset($_GET['search'])){echo $_GET['search']; } ?>" class="fruits-veggies-searchbar input" placeholder="Chercher un aliment" />

        </form>
        <span class="fruits-veggies-text02">
          (Vous ne trouvez pas ce que vous cherchez ? - Entrez votre propre
          repas personnalisé <a href="add-meal-per.php">ici</a>)
        </span>
        <table class="fruits-veggies-list list">
          ...
          if(isset($_GET['search']))
          {
              ...
              $query = "SELECT * FROM cata WHERE categorie='fruitlegume' and aliment LIKE '%$filtervalues%' ";
          ...
          $sql= "SELECT * FROM cata WHERE categorie='fruitlegume'; ";
